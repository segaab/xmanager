...
# Defaults / safe initializations used outside the run block
model_fname = None
metrics = {}
trades = pd.DataFrame()
latest_health = 0.0
...
if run:
    ...
    # downsample to daily for health gauge
    try:
        ...
        st.error(f"Error resampling bars to daily: {e}")
        st.stop()
    ...
    # Ensure candidates has expected columns
    ...
    st.write("Label value counts:", clean_candidates['label'].value_counts() if not clean_candidates.empty else {})
    st.write("NaN counts in important fields:", clean_candidates[feat_cols + ['label']].isna().sum().to_dict())

    if clean_candidates is None or clean_candidates.empty:
    ...
    if not (trades is None) and not trades.empty:
        ...
        try:
            ...
            # If grouping/plotting fails, still show table
            pass
    else:
    ...
    if st.button("Save logs to Supabase"):
        ...
        metadata = {
            ...
            "interval": interval,
            "feature_cols": feat_cols,
            "model_file": model_fname if model_fname is not None else None,
        ...
        backtest_metrics = {
            "num_trades": int(num_trades) if (trades is not None and not trades.empty) else 0,
            "total_pnl": float(total_pnl) if (trades is not None and not trades.empty) else 0.0,
            "avg_ret": float(avg_ret) if (trades is not None and not trades.empty) else 0.0,
            "win_rate": float(win_rate) if (trades is not None and not trades.empty) else 0.0,
            "latest_health": float(latest_health),
        }
        combined_metrics = dict(metrics) if metrics is not None else {}
